...
function agregarAlTicket() {
    ...
    if (!productos[codigo]) return alert("Producto no encontrado.");
    if (isNaN(kilos) || kilos <= 0) return alert("Cantidad inválida.");
    if (productos[codigo].stock < kilos) return alert("Stock insuficiente.");

    const producto = productos[codigo];
    const subtotal = kilos * producto.precio;
    productos[codigo].stock -= kilos;

    carrito.push({ codigo, nombre: producto.nombre, kilos, precio: producto.precio, subtotal });
